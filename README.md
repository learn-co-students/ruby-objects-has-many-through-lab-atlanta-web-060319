...
- The `Artist` class needs a class variable `@@all` that begins as an empty array XXXX
- The `Artist` class needs a class method `.all` that lists each artist in the 
  class variable XXXX
- An artist is initialized with a name and is saved in the `@@all` array. XXXX
...
- The `Artist` class needs an instance method, `#genres` that iterates over that
  artist's songs and collects the genre of each song.XXXX

**The `Song` model:**

- The `Song` class needs a class variable `@@all` that begins as an empty array.XXXX
- The `Song` class needs a class method `.all` that lists each song in the classXXXX
  variable.
- A song should be initialized with a name, an artist, and a genre, and be saved
  in the `@@all` array.XXXX

**The `Genre` model:**

- The `Genre` class needs a class variable `@@all` that begins as an empty array.XXXX
- The `Genre` class needs a class method `.all` that lists each genre in the class
  variable.XXXX
- A genre should be initialized with a name and be saved in the `@@all` array.XXXX
- The `Genre` class needs an instance method, `#songs`, that iterates through all
  songs and finds the songs that belong to that genre.XXXX
